...
function AddressInput(props: any) {
  ...
  const updateAddress = useCallback(
    async (value) => {
      ...
      if (isSupportedDomain(value)) {
        ...
        if (value.endsWith(".eth")) {
          processPromise( web3 && (web3 as any).eth.ens.getAddress(value));
        } else {
          processPromise(
            resolveDomain({
              domain: value,
    ...
    [resolveDomain, (web3 as any)?.eth?.ens]
  );
